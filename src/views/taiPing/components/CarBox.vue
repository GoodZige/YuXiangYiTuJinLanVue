<template>
  <div class="carBoxStyle">
    ...
    <div v-if="carInfo.sj" class="plEvent">
      ...
      <!-- 事件盒子 -->
      <div v-if="carInfo.sj" :class="`event-box-${isTop ? 'top' : 'down'}`">
        <div :class="`line-${isTop ? 'top' : 'down'}`"></div>
        <div v-if="carInfo.sj" class="box-one">
          <img :src="chetou" alt="">
          <img :src="paizhao" alt="">
          <div>{{ carInfo.cp }}</div>
...
export default {
  data() {
    return {
      chetouDefault: require('@/assets/images/暂无数据_左.png'),
      eventDefault: require('@/assets/images/暂无数据_右.png'),
      paizhaoDefault: require('@/assets/images/车牌底图.png'),
      carMap: {
        1: 'xiaoke',
        2: 'dahuo',
        3: 'daba',
  ...
  computed: {
    chetou() {
      return this.carInfo.chetouPic ? this.carInfo.chetouPic : this.chetouDefault
    },
    event() {
      return this.carInfo.eventPic ? this.carInfo.eventPic : this.eventDefault
    },
    paizhao() {
      return this.carInfo.paizhaoPic ? this.carInfo.chetouPic : this.paizhaoDefault
    }
  },
}
</script>
